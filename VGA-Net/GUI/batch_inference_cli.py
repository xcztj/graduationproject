...
使用方式:
    ...
    python batch_inference_cli.py --model ../Train/best_model_final.pt \
        --input ../DRIVE/test/preprocessed \
        --mask ../DRIVE/test/1st_manual \
        --output ../results
"""
...
def main():
    parser = argparse.ArgumentParser(description='VGA-Net 批量推理')
    parser.add_argument('--model', type=str, default='../Train/best_model_final.pt',
                        help='模型权重路径')
    parser.add_argument('--input', type=str, required=True,
                        help='输入图像文件夹（包含 .tif 文件）')
    parser.add_argument('--mask', type=str, default=None,
                        help='Ground Truth 文件夹（可选）')
    ...
    inferencer = VGAInferencer(args.model)
    ...
    for idx, img_path in enumerate(image_paths, 1):
        ...
        img_rgb, pred_prob, pred_binary = inferencer.predict(img_path)
